...
const (
	...
	HTTP_SERVICE_NAME = "tech-ngs-hope-http"
	SERVICE_VERSION   = "1.0"
	HTTP_SERVICE_PORT = ":8301"
)
...
func App(mEngine *gin.Engine) web.Service {
	if tracer == nil {
		log.Fatal("[Loading] Tracer must be inited before init tracer")
	}

	srv := web.NewService(
		web.Name(HTTP_SERVICE_NAME),
		web.Address(HTTP_SERVICE_PORT),
		web.Handler(mEngine),
		web.Name(HTTP_SERVICE_NAME),
	)
