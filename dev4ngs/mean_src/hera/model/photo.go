...
func PhotoToData(mdb *mgo.Database, p *dream.Photo, o *PhotoDataOption) (Dungeons, error) {
	d := make(Dungeons)
	d[`id`] = p.ID
	d[`uuid`] = p.UUID
	...
	if p.User != nil {
		uData, err := UserToData(mdb, p.User, &UserDataOption{FillSensitive: o.FillSensitive})
		if err != nil {
			return nil, err
		}
