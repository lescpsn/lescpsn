...
func SetActiveAvatarWithTime(MDB *mgo.Database, uUUID string, a *UserAvatar, time time.Time) (*UserAvatar, error) {
	if err := AvatarsCollection(MDB).Update(bson.M{"user_uuid": uUUID, "_id": a.ID}, bson.M{
		"$set": bson.M{
			"active":    true,
			"timestamp": time,
...
//func GetUserAvatarsWithFemale(MDB *mgo.Database, uUUID uuid.UUID) ([]*UserAvatar, error) {
//	var avatars []*UserAvatar
//	if err := AvatarsCollection(MDB).Find(bson.M{"user_uuid": uUUID.String(), "type": bson.M{"$ne" : AvatarTypeSystem}}).Sort("-timestamp").All(&avatars); err != nil {
//		if err == mgo.ErrNotFound {
//			return nil, nil
//		}
//		return nil, err
...
//}
//
//func GetUserAvatarsWithMale(MDB *mgo.Database, uUUID uuid.UUID) ([]*UserAvatar, error) {
//	var avatars []*UserAvatar
//	if err := AvatarsCollection(MDB).Find(bson.M{"user_uuid": uUUID.String(), "type": bson.M{"$ne" : AvatarTypeSystemFemale}}).Sort("-timestamp").All(&avatars); err != nil {
//		if err == mgo.ErrNotFound {
//			return nil, nil
//		}
//		return nil, err
...
func GetSystemAvatars(MDB *mgo.Database, uUUID string) (*UserAvatar, error) {
	var avatar *UserAvatar
	if err := AvatarsCollection(MDB).Find(bson.M{"user_uuid": uUUID, "type": AvatarTypeSystem}).One(&avatar); err != nil {
		if err == mgo.ErrNotFound {
			return nil, nil
		}
		return nil, err
