...
func composeThumbnailURLV2(url string, width, height int64) string {
	...
	if tsEdgeMode == tsEdgeTypeWidth {
		if ratio := float32(height) / float32(width); ratio < 16.0/9.0 {
			if width > 720 {
				url = url + "?imageMogr2/auto-orient/thumbnail/720x"
			}
		} else {
			if height > 1280 {
				url = url + "?imageMogr2/auto-orient/thumbnail/x1280"
			}
		}
	}
	...
	if tsEdgeMode == tsEdgeTypeHeight {
		if ratio := float32(width) / float32(height); ratio < 16.0/9.0 {
			if height > 720 {
				url = url + "?imageMogr2/auto-orient/thumbnail/x720"
			}
		} else {
			if width > 1280 {
				url = url + "?imageMogr2/auto-orient/thumbnail/1280x"
			}
		}
	}
...
func composeWaterFallURL(url string, width, height int64) string {
	...
	if fileRatio < 0.5 || fileRatio > 2.5 {
		if tsEdgeMode == tsEdgeTypeWidth {
			...
			if width < 50 {
				url = url + "?imageMogr2/auto-orient/thumbnail/50x"
			}
		}
		...
		if tsEdgeMode == tsEdgeTypeHeight {
			...
			if height < 50 {
				url = url + "?imageMogr2/auto-orient/thumbnail/x50"
			}
		}
		...
		if tsEdgeMode == tsEdgeTypeWidth {
			if width > 720 {
				url = url + "?imageMogr2/auto-orient/thumbnail/720x1800"
			}
			if width > 50 && width < 720 {
				url = url + "?imageMogr2/auto-orient/crop/x" + strconv.Itoa(int(float32(width)*2.5))
			}
			if height < 50 {
				url = url + "?imageMogr2/auto-orient/thumbnail/50x150"
			}
		}

		if tsEdgeMode == tsEdgeTypeHeight {
			if height > 720 {
				url = url + "?imageMogr2/auto-orient/thumbnail/1800x720"
			}
			if height > 50 && width < 720 {
				url = url + "?imageMogr2/auto-orient/crop/" + strconv.Itoa(int(float32(height)*2.5)) + "x"
			}
			if height < 50 {
				url = url + "?imageMogr2/auto-orient/thumbnail/150x50"
			}
		}
	}
...
func composeSquareURL(url string, width, height int64) string {
	...
	if tsEdgeMode == tsEdgeTypeWidth {
		if width > 200 {
			url = url + "?imageMogr2/auto-orient/thumbnail/!200x200r/gravity/center/crop/200x200"
		} else {
			wxh := strconv.Itoa(int(width)) + "x" + strconv.Itoa(int(width))
			url = url + "?imageMogr2/auto-orient/thumbnail/!" + wxh + "r/gravity/center/crop/" + wxh
		}
	}
