...
import (
	...
	loger "git.ngs.tech/mean/houston/log"
...
func InitHandler(husConf *config.HustonConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		DB := GetX(config.GetDreamConf(), nil)
		transaction := Begin(DB)
		MDBSession, MDB := GetMDBAndMSession(config.GetMongoConf())
		m := NewMeanController(transaction, MDBSession, MDB, defaultBroker)
...
func AuthorizationHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		uDeviceToken, err := GetDeviceTokenByUser(uID)
		if err != nil {
...
func LoggerWithWriter() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		loger.ToEchoLogFile("logs", byt.Bytes(), 1024*1024*5)
