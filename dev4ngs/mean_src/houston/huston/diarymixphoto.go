...
func (m *MeanController) GetDiaryMixPhotoByUserUUID(useruuid, id string, pageSize int, isDropRefresh bool) ([]Dungeons, error) {
	...
	if id == "" {
		bm = bson.M{
			...
			"$or": []bson.M{
				bson.M{
					"data.privacy": DiaryPrivacy_diary_privacy_public.String(),
					"data.status":  DiaryStatus_diary_status_published.String(),
				},
				bson.M{"data.privacy": DiaryPrivacy_diary_privacy_public.String()},
			}}
		bm = bson.M{"user_uuid": useruuid, "$or": []bson.M{bson.M{"data.privacy": DiaryPrivacy_diary_privacy_public.String(), "data.status": DiaryStatus_diary_status_published.String()}, bson.M{"data.privacy": PhotoPrivacy_photo_privacy_public.String()}}}
	} else {
		...
		if isDropRefresh {
			...
			bm = bson.M{"_id": bson.M{"$lt": oid}, "user_uuid": useruuid, "$or": []bson.M{bson.M{"data.privacy": DiaryPrivacy_diary_privacy_public.String(), "data.status": DiaryStatus_diary_status_published.String()}, bson.M{"data.privacy": PhotoPrivacy_photo_privacy_public.String()}}}
		}
	}

	qy := DiarymixphotoCollection(m.MDB).Find(bm).Sort("-timestamp").Limit(pageSize)
	if err := qy.All(&dmp); err != nil {
		return nil, err
	}

	diary_uuids := []string{} // 将mongodb中多条日志数据uuid组装成数组diary_uuids,用diary_uuids数组一次查询
	photo_uuids := []string{} // 将mongodb中多条图片数据uuid组装成数组photo_uuids,用photo_uuids数组一次查询
	for _, t := range dmp {
	...
	if len(photo_uuids) != 0 {
		psRsp, err := Cl.FindPhotoByUUIDs(context.TODO(), &UUIDsWithEchoOptionRequest{
			UUIDs:           photo_uuids,
			PhotoEchoOption: &PhotoEchoOption{FetchNote: true, FetchBasePhoto: true},
		})
		if err != nil {
			return nil, err
		}
		ps := psRsp.Photos
		for _, t := range ps {
			d, _ := PhotoToData(t, &PhotoDataOption{FillSensitive: true})
			dataMap[t.UUID] = d
		}
	}
	...
	for _, t := range dmp {
		data_uuid := t.Data[`uuid`].(string)
		if v, exist := dataMap[data_uuid]; exist {
			t.Data = v.(Dungeons)
			d, err := t.ToData()
			if err != nil {
				continue
