...
var ProductPanel = React.createClass({
    ...
    onSetStatus: function (index, status) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.product) {
                    this.state.product_list[index] = data.product;
    ...
    onEditPriceFinish: function (index, value) {
        ...
        var v = Math.round(parseFloat(value) * 10000);
        var p = this.state.product_list[index];
        var data = JSON.stringify({'id': p['id'], 'value': v});
    ...
    render: function () {
        return (
            <section className="wrapper">
                <QueryPanel onLoad={this.loadProductList}
...
var ProductList = React.createClass({
    ...
    editPriceFinish: function (index) {
        var value = $('#edit' + index).val();
        this.props.onEditPriceFinish(index, value);
    },
    ...
    render: function () {
        var productNode = this.props.product_list.map(function (product, index) {
                ...
                if (product.status == 'enabled') {
                    statusNode = (
                        <a href="javascript:void(0);" className="list_open"
                           onClick={this.setStatus.bind(this, index, 'disabled')}>关闭</a>
                    );
                } else if (product.status == 'disabled') {
                    statusNode = (
                        <a href="javascript:void(0);" className="list_open"
                           onClick={this.setStatus.bind(this, index, 'enabled')}>开启</a>
                    );
                } else {
                    statusNode = '';
                }

                var status_color = product.status == 'disabled' ? 'text-center price_color' : 'text-center';
                ...
                return (
                    <tr>
                        ...
                        <td className={status_color}>{product.status_n}</td>
