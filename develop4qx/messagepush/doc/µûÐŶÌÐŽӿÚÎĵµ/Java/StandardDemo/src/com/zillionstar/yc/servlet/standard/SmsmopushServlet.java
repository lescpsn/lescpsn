 ...
 *
 */
public class SmsmopushServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException,IOException {
        ...
        try {
            ...
            if (parm[0].split(":")[0].trim().replace("\"", "").equals("Msg_Id") &&
                ...
                parm[2].split(":")[0].trim().replace("\"", "").equals("TP_pId") &&
                parm[3].split(":")[0].trim().replace("\"", "").equals("TP_udhi") &&
                parm[4].split(":")[0].trim().replace("\"", "").equals("Msg_Fmt") &&
                parm[5].split(":")[0].trim().replace("\"", "").equals("Src_terminal_Id") &&
                parm[6].split(":")[0].trim().replace("\"", "").equals("Msg_Content") 
            		) {
                out.print("{Rspcode:0}");
            } else {
                out.print("{Rspcode:1}");
