...
//            out.println("Post Success!");
    ...
    public static String postMourlverify() {
        ...
        try {
            URL realUrl = new URL(enterpriseUrl);
            // 打开和URL之间的连接
            httpURLConnection = (HttpURLConnection) realUrl.openConnection();
            // 设置通用的请求属性
            httpURLConnection.setRequestProperty("accept", "application/json");
...
<%
    ...
    } else if(m.equals("mourlverify")) {
        out.println(postMourlverify());
    } else if(m.equals("smsrptpush")) {
        out.println(postSmsrptpush());
    } else if(m.equals("smsmopush")) {
        out.println(postSmsmopush());
    }
%>